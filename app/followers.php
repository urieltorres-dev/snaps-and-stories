...
$sqlCmd = "SELECT u.* FROM usuarios u JOIN seguidores s ON u.id = s.usuario_seguidor_id WHERE s.usuario_siguiendo_id = ? AND s.eliminado = ?"; // Sentencia SQL a ejecutar.
...
$followers = $stmt->fetchAll();  // Obtenemos el resultado de la consulta.
$numeroFollowers = count($followers); // Obtenemos el número de usuarios que nos siguen.
