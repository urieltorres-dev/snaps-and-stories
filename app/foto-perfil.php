...
$db2 = getDbConnection();  // Obtenemos el objeto PDO que es con el que vamos a interactuar con DB.
$sqlCmd2 = "UPDATE usuarios SET foto_perfil = ? WHERE id = ?";   // <-- Parámetros de la sentencia.
$params2 = [     // Especificación de los parémtros de la sentencia SQL del Insert.
...
$stmt2 = $db2->prepare($sqlCmd2);  // Obtenemos el objeto Statement que representa la ejecución de la senttencia SQL.
$stmt2->execute($params2);   // Realizamos la ejecución especificando los parámetros.

$_SESSION['Usuario_Foto_Perfil'] = $secureId;
...
$params = [     // Especificación de los parémtros de la sentencia SQL del Insert.
    ...
    NULL,  // descripcion -> descripción del archivo, si se especificó, guardamos el trim (quita espacios en blanco al principio y al final), de otra forma guardamos null.
